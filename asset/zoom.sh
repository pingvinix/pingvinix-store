...
приложение. Единый корпоративный интерфейс для всех вариантов
использования. Разработан и оптимизирован для надежной работы
...
ПРИМЕЧАНИЕ: Эта оболочка не проверена, не связана и
не поддерживается zoom.us.' \
       /tmp/description.png \
...
xlunch --window --title "PingviniX Store"  \
       ...
       --button "/tmp/description.png;;425,0;null" \
       --button "/opt/appstore/img/img_delete.png;;25,300;/tmp/store/asset/zoom_delete.sh" \
       --button "/opt/appstore/img/img_update.png;;225,300;/tmp/store/asset/zoom_update.sh" \
       --button "/tmp/store/asset/image/zoom_scr.png;;1,400;null" \
)
fi
